...
- **Blinkit fills the whole cart in ~1 second.** Blinkit keeps the live cart entirely in the page's own `localStorage.cart` — adding an item fires no server request; the app syncs to the account cart at checkout. So CookCart looks every ingredient up via the search API in parallel (bounded concurrency to stay under the rate limit), ranks the best match, builds the whole cart object and writes it once, then reloads so the app hydrates it. A full cart lands in ~1s instead of minutes, on the tab you're already on, with no per-item DOM automation to stall. Zepto/Instamart still use the step-wise DOM flow (search → click → verify). The reverse-engineering that made this possible is in [`instrument/`](instrument/).
